...
class CmdArgs:
    def __init__(self, config, init, name=None, query=None, input_key=None, output_key=None, kwargs={}):
...
def parse_tests_args():
    ...
    parser.add_argument('-c', '--config', type=str, required=False, default=None, help="specify config file, if not set, run with all test configs")
    parser.add_argument('-i', '--init', type=str, required=False, default=None, help="specify test name")
    parser.add_argument('-l', '--list', action='store_true', help="list all test configs")
    parser.add_argument('-r', '--root', type=str, required=False, default=None, help="specify root config directory")
...
# an example
tests = [
    CmdArgs("graph_simple_as_agent.yaml", "graph_simple", 
...
def _log(logger: logging.Logger, level, msg, args, exc_info=None, 
         extra=None, stack_info=False,
         stacklevel=1) -> logging.LogRecord:
    """
...
def run_test(cmd_args: CmdArgs, collector: list[dict]):
    ...
    try:
        ...
        rec = _log(logger, logging.ERROR, e, args=[], exc_info=e, stack_info=True, stacklevel=3)
        _append_results(collector, cmd_args.config, cmd_args.name, error=e, error_msg=_format(rec))
        logger.handle(rec)
...
def list_tests_with_files(file_path=None,):
    if file_path:
        files = [file_path]
    else:
        test_dir = os.path.dirname(__file__)
