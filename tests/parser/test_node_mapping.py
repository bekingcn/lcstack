...
def test_om_non_struct_pass_through():
    ...
    runnable = make_node_with_mapping(output_mapping={None: {"name": None, "output_type": "pass_through"}})
...
def test_om_non_struct_to_primitive():
    # TODO: improve this config, add MappingParserArgs for input and output mapping
    # like: output_mapping={"output_type": "primitive"}
    runnable = make_node_with_mapping(output_mapping={None: {"name": None, "output_type": "primitive"}})
    from langchain_core.messages import AIMessage
...
def test_om_non_struct_to_struct_2():
    runnable = make_node_with_mapping(output_mapping={"result": {"name": None, "output_type": "primitive"}})
    from langchain_core.messages import AIMessage
    result = runnable.invoke(input=AIMessage(content="bar"))
    print("==>", result)
...
def test_im_om_simple():
    runnable = make_node_with_mapping(input_mapping={"foo": None}, output_mapping={"foo2": "foo"})
