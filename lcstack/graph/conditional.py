        ...
        if isinstance(self.conditions, str):
            ...
            if get_expr_enabled():
                ...
                if eval_result is None:
                    raise ValueError(
                        f"failed to evaluate expression or not a valid expression: {self.conditions}"
                    )
                return eval_result
        else:
            return any(
                [
                    c["property"] in state and state[c["property"]] == c["value"]
...
class BranchsVertex(BaseVertex):
    ...
    def match_and_set(self, state) -> tuple[str, Dict[str, Any]]:
        ...
        for setter in setters:
            output[setter["property"]] = ConditionalBranch.parse_value(
                state, setter["value"]
            )
...
class ConditionalWorkflow(Workflow):
    ...
    def _branch_func(self, branch_vertex: BranchsVertex, this_name: str):
        def route_to_node(state):
            ...
            if (
                branch_steps
                and len(branch_steps) > 0
                and branch_steps[-1][0] == this_name
            ):
        ...
        if default_node:
            ...
            raise ValueError(
                f"no default `branch` (or `next`) specified in vertex {this_name}"
            )
    ...
    def build_graph(self):
        ...
        for v in reversed(self.vertices):
            ...
            if isinstance(v, CallableVertex):
                ...
                if next_node_name not in _edges:
        ...
        if start_node_name not in _edges:
            ...
            if isinstance(v, BranchsVertex):
                ...
                if self._need_setters(branchs) or v.default_setters:
                    ...
                    for from_node_name in from_node_names:
                        self._add_setters_and_conditional_edges(
                            graph, from_node_name, node_name, v
                        )
                else:
                    for from_node_name in from_node_names:
