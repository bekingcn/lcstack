...
class ChatHistoryFactory:
    def __init__(self, provider: str, **kwargs):
        ...
        self.kwargs = kwargs
        self.history_constructor, self.history_kwargs = _create_chat_history_internal(provider, **kwargs)

    def get_by_session_id(self, session_id: str) -> BaseChatMessageHistory:
        return self.history_constructor(session_id=session_id, **self.history_kwargs)
...
def _create_chat_history_internal(provider: str, **kwargs):
    if provider == "cassandra":
        ...
        inmemory_history = GLOABLE_INMEMORY_CHAT_HISTORY_FACTORY

        def get_history(session_id: str):
            if session_id in inmemory_history:
                return inmemory_history[session_id]
            else:
                inmemory_history[session_id] = (
