...
from .container import (
    CHAT_HISTORY_PARAM_NAME,
    BaseContainer,
    NoneRunnableContainer,
    RunnableContainer,
)
from .models import (
    InitializerConfig,
    InitializerDataConfig,
    NonRunnables,
)
...
class DpendencyInfo(BaseModel):
    ...
    def from_updated(self) -> bool:
        return (
            self.updated_data_kwargs is not None and len(self.updated_data_kwargs) > 0
        )
...
class BaseInitializer(BaseModel):
    ...
    def _init_child(self, name: str):
        ...
        cls: BaseInitializer = initializer_class_map.get(
            child_component.component_type, ComponentInitializer
        )

        child = cls.from_config(
            child_component_config, parent=self, path=self.path.child([name])
        )
        child.parse_config()
    ...
    def _parse_value(self, _key, _value, initializer_data, levels=None):
        ...
        # ref a initializer case 2: {"initializer_name": "...", "initializer_data": {...}}
            ...
            and "initializer_name" in _value
            and "initializer_data" in _value
        ):
            ...
            if args:
                for ik, iv in args.items():
                    args[ik] = self._parse_value(
                        f"{_key}${ik}", iv, initializer_data, levels + [cp_name]
                    )
        # TODO: to be checked. casscade parsing
        elif isinstance(_value, dict):
            new_values = {}
            for vk, vv in _value.items():
                new_values[vk] = self._parse_value(
                    f"{_key}${vk}", vv, initializer_data, levels
                )
            return new_values
            ...
            for index, v in enumerate(_value):
                ...
                new_values.append(
                    self._parse_value(f"{_key}${index}", v, initializer_data, levels)
                )
            return new_values
        else:
            return _value
        ...
        if args:
            ...
            dep_info = DpendencyInfo(
                name=_key, path=initializer.path, initializer=initializer
            )
...
class RootInitializer(BaseInitializer):
    ...
    def from_config(cls, config: Dict[str, Any]):
        ...
        return super().from_config(
            root_config, parent=None, path=InitializerPath.root()
        )
