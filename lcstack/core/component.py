...
class Component(BaseModel):
    ...
    inputs: Union[str, list[str], dict[str, Union[str, NamedMappingParserArgs]]] = (
    ...
    default_output_parser_args: MappingParserArgs = (
        default_output_parser_args_with_type(OutputParserType.pass_through)
    )
        ...
        if isinstance(inputs, str):
            self.inputs = {
                None: NamedMappingParserArgs(
                    name=inputs, output_type=OutputParserType.pass_through
                )
            }
        elif isinstance(inputs, list):
            self.inputs = {
                v: NamedMappingParserArgs(
            ...
            for k, v in inputs.items():
                if isinstance(v, str):
                    inputs[k] = NamedMappingParserArgs(
                        name=v, output_type=OutputParserType.pass_through
                    )
